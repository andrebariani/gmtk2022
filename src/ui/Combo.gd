...
func trigger_combo():
	...
	tween.interpolate_property(popper, "rect_position", 
		Vector2(0, 0), Vector2(305, 0), 0.4, Tween.TRANS_QUAD, Tween.EASE_IN, 0.1)
	tween.interpolate_property(popper, "rect_position", 
		Vector2(305, 0), Vector2(0, 0), 0.25, Tween.TRANS_BACK, Tween.EASE_IN, 0.55)
	
	tween.interpolate_property(popper, "rect_rotation", 
		0, 360, 0.5, Tween.TRANS_BACK)
	tween.start()
